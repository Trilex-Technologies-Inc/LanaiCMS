...
$ajax->setJSCode("mybutton",$ajax->htmlDisable("mybutton").$ajax->htmlEnable("stoptimer"),$ajax->htmlEnable("mybutton")); 
/*  Here we add some JavaScript to execute before and after the AJAX request, notice that we may type manually 
	JavaScript code, ir call the build in functions that generates the appropiate JavaScript code, in this case
	to disable and enable buttons.
*/
?>
<html>
	<head>
		<title>AjaxCore 1.1.2 » BindPeriodicalTimer Example</title>
	...
	<body>
		<table align="center" width="740">
			...
			<tr>
				<td class="main">
					<h2>BindPeriodicalTimer Example</h2>
					This is a simple example file to check out the <a href="http://ajaxcore.sourceforge.net">AjaxCore</a> framework. <br /><br />
					When the button is pressed,a <b>Timer</b> starts with the milliseconds specified on the bindTimer call, and when it get to zero, an AJAX request is being made to return a random number. And it will keep repeating until we stop it<br />
					<br />
					In this case we'll use a <b>bindPeriodicalTimer</b> between an HTML object and a PHP function.  <br /> <br />
					...
					Press <a href="bindPeriodicalTimer.phps">here</a> to download this page source file, also check the stuff below for more information.<br />
