...
/*
 *  4/28/2004
 *  Modified by Anoochit Chalothorn (anoochit@redlinesoft.net)
 *
 *	http://www.redlinesoft.net
 *
*/
...
class feedParser {
	...
	function getRSSChannel($data) {
				...
				if(is_array($child)) {
					switch($child['tag']) {
						...
						case "RSS2:DESCRIPTION":
							$channel['description'] = $child['children'][0];
							break;
						/// 4/28/2004 ANOOCHIT ADD Element image to array
						case "UNDEF:IMAGE":
						case "RSS:IMAGE":
						case "RSS2:IMAGE":
		...
		/// 4/28/2004 ANOOCHIT ADD Element image to array
		return array('channel' => $channel, 'image' => $image,'item' => $item);
	}
	...
	/// 4/28/2004 ANOOCHIT ADD getRSSImage function
	function getRSSImage($data) {
		if(is_array($data['children'])) {
			foreach($data['children'] as $child) {
				if(is_array($child)) {
					switch($child['tag']) {
						...
						case "RSS:TITLE":
						case "RSS2:TITLE":
							$image['title'] = $child['children'][0];
							break;
						case "UNDEF:LINK":
						case "RSS:LINK":
						case "RSS2:LINK":
							$image['link'] = $child['children'][0];
							break;
						case "UNDEF:DESCRIPTION":
						case "RSS:DESCRIPTION":
						case "RSS2:DESCRIPTION":
							$image["description"] = $child['children'][0];
							break;
						case "UNDEF:URL":
						case "RSS:URL":
						case "RSS2:URL":
							$image["url"] = $child['children'][0];
							break;
						case "UNDEF:WIDTH":
						case "RSS:WIDTH":
						case "RSS2:WIDTH":
							$image["width"] = $child['children'][0];
							break;						
						case "UNDEF:HEIGHT":
						case "RSS:HEIGHT":
						case "RSS2:HEIGHT":
							$image["height"] = $child['children'][0];
							break;			
						case "UNDEF:PUBDATE":
						case "RSS:PUBDATE":
						case "RSS2:PUBDATE":
							$image["date"] = strtotime($child['children'][0]);
							$image["locdate"] = strtotime($child['children'][0]);
							break;
						default:
							break;
					}
