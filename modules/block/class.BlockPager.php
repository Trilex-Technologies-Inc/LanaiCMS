...
/**
	...
	class BlockPager extends ADODB_Pager {
		...
		function RenderGrid()
		{
			...
			<?php global $sys_lanai; $sys_lanai->renderCsrfField('block'); ?>
			<tr>
			...
			<tr>
				...
				<?php
					if ($this->rs->fields['blcActive']=='y') {
					    ?>
						<button type="submit" onclick="return prepareActiveToggle('<?=$this->rs->fields['blcId']; ?>','n');" style="border:0;background:none;padding:0;cursor:pointer;">
						<img src="theme/<?=$mod_lanai->cfg['theme'];?>/images/ok.gif" border="0" align="absmiddle">
						</button>
						<?php
					} else {
						?>
						<button type="submit" onclick="return prepareActiveToggle('<?=$this->rs->fields['blcId']; ?>','y');" style="border:0;background:none;padding:0;cursor:pointer;">
